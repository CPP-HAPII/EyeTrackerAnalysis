...
def collapse_to_fixations(input_file, output_file):
    ...
    """
    print("Collapsing fixation file...")
    ...
    if overwrite.lower() == "y":
        ...
        for index, row in row_iterator:
            # Only care about rows with a valid FPOGID value
            cur_fpog_id = row['FPOGID']
            cur_fpog_valid = row['FPOGV']            

            last_fpog_id = last['FPOGID']
            last_fpog_valid = last['FPOGV']   

            if last_fpog_valid and not cur_fpog_valid:
                new_df_len = len(new_df.index)
                new_df.loc[new_df_len] = last
